...
// Leaflet divIcon anchored precisely at the center of the truck graphic [27, 27]
const vehicleIcon = L.divIcon({
    className: 'custom-vehicle-truck-marker bg-transparent border-0',
    html: `
        <div style="position: relative; width: 54px; height: 54px; display: flex; align-items: center; justify-content: center;">
            <div style="position: absolute; bottom: 50px; left: 50%; transform: translateX(-50%); background-color: #0284C7; color: white; padding: 2px 6px; border-radius: 6px; font-weight: 700; font-size: 9px; font-family: system-ui, sans-serif; white-space: nowrap; box-shadow: 0 2px 5px rgba(0,0,0,0.35); border: 1.5px solid white;">
                DRIVER
            </div>
            <img src="${TruckLogo}" alt="Driver" style="width: 48px; height: 48px; object-fit: contain; filter: drop-shadow(0px 3px 6px rgba(0,0,0,0.4));" />
        </div>
    `,
    iconSize: [54, 54],
    iconAnchor: [27, 27], // Exact center of truck image aligns with LatLng dot
    popupAnchor: [0, -27],
});
